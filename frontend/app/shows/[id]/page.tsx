    ...
    useEffect(() => {
        const fetchShowSeasons = async () => {
            try {
                ...
                console.error("Failed to fetch show seasons:", e);
            } finally {
                setLoading(false);
            }
        };
                    ...
                    {loading ? (
                        ...
                        <Select
                            ...
                            sx={{
                                ...
                                "&.Mui-focused .MuiOutlinedInput-notchedOutline": { borderColor: "white", borderWidth: 1 },
                            }}
                        >
                            {seasons.map(season => (
                    ...
                    {currentSeason?.episodes.map((episode) => (
                        <Link href={`/player?video-id=${episode.id}`} key={episode.id}>
                            <Box className="show-episode-container">
                                ...
                                <Image src={episode.stillPath} alt={`Episode ${episode.episodeNumber}`} width={200} height={100} style={{ borderRadius: "6px", marginRight: 50 }} />
                                <Box>
